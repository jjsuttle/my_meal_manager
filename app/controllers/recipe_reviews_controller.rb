class RecipeReviewsController < ApplicationController
  before_action :current_user_must_be_recipe_review_commentor, only: [:edit, :update, :destroy] 

  before_action :set_recipe_review, only: [:show, :edit, :update, :destroy]

  # GET /recipe_reviews
  def index
    @q = current_user.recipe_comments.ransack(params[:q])
    @recipe_reviews = @q.result(:distinct => true).includes(:commentor, :recipe).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_recipe_review_commentor
    set_recipe_review
    unless current_user == @recipe_review.commentor
    ...
    def recipe_review_params
      params.require(:recipe_review).permit(:reviewer_id, :recipe_id, :rating, :review)
    end
end
